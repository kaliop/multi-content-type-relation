...
export const TableItem = ({ entry, type, uniqueId, disabled, onAdd, onDelete }: Props) => {
  ...
  return (
    <Tr style={style} {...attributes} {...listeners} data-tableitem={entryIdentifier}>
      <Td>{type === "selected" ? <IconButton noBorder>
        <Drag />
      </IconButton> : null}</Td>
      <Td>
      ...
      <Td>
        <Flex>
            ...
            {type === "suggestion" ? (
              <IconButton label="Add" onClick={() => onAdd!(entry)} disabled={disabled}>
                <Plus />
              </IconButton>
            ) : type === "selected" ? (
